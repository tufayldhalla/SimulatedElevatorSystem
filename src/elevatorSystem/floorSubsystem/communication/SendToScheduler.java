...
/**
 * Sends requests from Floor Subsystem to Scheduler Subsystem
 * 
 * @author L4G3
 * @version 2.0
 */
public class SendToScheduler {
	...
	/**
	 * Scheduler's Floor-Listener's port number
	 */
	...
	/**
	 * Creates a new send socket and IP address
	 * @param sAddress is the address of the scheduler subsystem
	 */
	public SendToScheduler(InetAddress sAddress, Speed s) {
	 ...
	 * IP address
	 */
	public SendToScheduler(Speed s) {
		speed = s;
		try {
	...
	public synchronized void send(byte[] msg) {
		 ...
		 * packet to Scheduler)
		 */
		System.out.print("Floor: sends request to Scheduler = \n\tbytes:");
