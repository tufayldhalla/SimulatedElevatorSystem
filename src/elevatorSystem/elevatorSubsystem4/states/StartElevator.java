...
public class StartElevator extends Thread {
	...
	public void run() {
		...
		if (fault == 3) {
			...
			index += newData.length;
			System.out
					.println("\n\n***********\nElevator " + elevatorID + ": FAULT FIXED, DOORS CAN CLOSE\n***********\n");
			sender.send(msg);
		...
		/*
		 * Start timer. If elevator motor not started then it must be stalled
		 */
//		try {
//			Thread.sleep(IntData.CHECK_FOR_STALL.getTime(elevatorSubsystem.getSpeed()));
//		} catch (InterruptedException e) {
//			e.printStackTrace();
//		}
